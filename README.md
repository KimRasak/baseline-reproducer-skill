...
| <strong>AA</strong> | 基本闭环 | 主链路完整，只有不改变科学含义的少量可追溯兼容处理或确定性重建步骤。 | 全部官方工件可用，但依赖库升级后旧参数名已失效；兼容补丁只把旧参数名换成新参数名，传入值和计算过程不变，也不改变模型与协议[^protocol]，重跑得到 `82.6`。 |
| <strong>A</strong> | 可执行但有重要限制 | 可以进行主张级验证，但缺少 seed、精确版本、checkpoint 选择或统计细节等重要信息，不能称严格复现。 | checkpoint、测试代码和数据可用，但作者没有公开 seed；采用预先登记的独立 seed 得到 `82.5`，只能称固定独立协议结果。 |
