...
四个指标分别评级，不能用一张通用表互相推导。评级从低到高为 `F`、`I`、`M-`、`M`、`M+`、`E`、`E+`、`O`；标签沿用“Failed / Improving / Medium / Excellent / Outstanding”的分级命名[[1](https://www.zhihu.com/question/495085246), [2](https://www.dingteam.com/article/285)]，这里衡量的是<strong>公开材料和实际复现为该主张数值提供了多强的可核查证据</strong>：评级会影响我们对该数值的可信程度，但不评价数值本身的高低，也不延伸为对整篇论文总体可信度的评级。`NR`（Not Rated / Not Applicable）表示尚未评级或不适用，不属于上述等级。
