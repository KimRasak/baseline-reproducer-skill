...
| <strong>A</strong> | 可执行但有重要限制 | 可以进行主张级验证，但缺少 seed、精确版本、checkpoint 选择或统计细节等重要信息，不能称严格复现。 | checkpoint、测试代码和数据可用，但作者没有公开 seed；采用预先登记的独立 seed[^independent-seed] 得到 `82.5`，只能称固定独立协议结果。 |
| <strong>BBB</strong> | 部分链路 | 能完成 evaluator replay、固定子集或独立协议验证，但无法闭合论文原指标。 | 作者只公开预测结果和评分代码；重新评分可以恢复 `82.7`，但没有对应 checkpoint，无法验证模型是否能产生这些预测。 |
...
[^independent-seed]: <strong>预先登记的独立 seed</strong>是指作者未公开原实验的随机种子时，复现者在运行前先固定并记录一组自选种子，例如 `42、43、44、45、46`，再按该方案执行。这样可以避免看到结果后只挑表现好的种子；但这些不是作者原来的种子，因此不能称为精确恢复作者实验。
