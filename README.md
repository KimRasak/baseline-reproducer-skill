...
交付完整度评级用于 `TD` 和 `RD`，只评价作者公开的材料能支持怎样的链路，不把复现者的实际执行结果计入评级。下面仍以 `Paper X` 的表 1 主张 `b1=82.7` 为例；表中的例子主要展示 `TD`，`RD` 按同一尺度评价从训练数据到权重的材料链路。

| 等级 | 链路完整度 | 交付含义 | 以 `Paper X / b1=82.7` 为例 |
|---|---|---|---|
| <strong>AAA</strong> | 完整闭环 | 所需工件、身份、协议、划分和聚合均已明确公开，可按作者原设定闭合链路。 | 作者公开了与表 1 精确对应的 checkpoint、完整测试划分、测试代码、seed 和聚合方式。 |
| <strong>AA</strong> | 基本闭环 | 主链路材料齐全，仅需不改变科学含义的少量可追溯兼容处理或确定性重建。 | 官方工件齐全，但测试清单需由公开脚本确定性重建，或测试代码只需修改旧版 API 调用[^api-call]。 |
| <strong>A</strong> | 可执行但有重要限制 | 已交付主张级验证所需的核心材料，但缺少 seed、精确版本、checkpoint 选择或统计细节等重要信息。 | checkpoint、测试代码和数据可用，但作者没有公开 seed，无法严格恢复原测试设置。 |
| <strong>BBB</strong> | 部分链路 | 公开材料只支持 evaluator replay、固定子集或独立协议，无法闭合论文原指标链路。 | 作者未公开 checkpoint，只公开测试集上的现成预测文件和评分代码；这些材料只能重算 `82.7`，不能验证模型是否会生成这些预测。 |
| <strong>BB</strong> | 冒烟级 | 公开材料最多支持代码导入、权重加载、单样本生成或 evaluator 接口检查。 | checkpoint 和示例脚本已公开，但没有完整测试划分或可执行的全量评测入口。 |
| <strong>B</strong> | 仅工件追踪 | 找到部分代码、权重或数据入口，但尚不能组成可执行的指标链路。 | 找到了代码仓库和一个权重链接，但无法确认该权重是否对应表 1，也没有测试命令。 |
| <strong>CCC</strong> | 严重缺失 | 关键权重、代码、数据、划分、配对键或 evaluator 缺失，公开材料不足以测试主张。 | 论文只写出 `82.7`；没有对应权重，测试划分或指标实现也未公开。 |
| <strong>D</strong> | 无效链路 | 已公开工件的身份或协议与目标主张冲突，不能归属于该主张。 | 下载链接实际指向另一个模型变体，或公开脚本使用不同的 `Dataset 1` 划分。 |
| <strong>NR</strong> | 未评级/不适用 | 尚未审计交付材料，或该交付维度对当前主张不适用。 | 已登记 `b1=82.7`，但尚未检查作者公开的代码、权重、数据或评测材料。 |

### 复现完整度等级定义

复现完整度评级用于 `TR` 和 `RR`，评价复现者实际执行和审计到哪里；材料已公开但尚未运行，不能据此获得高复现评级。下表继续使用同一主张，例子主要展示 `TR`，`RR` 按同一尺度评价从训练数据到新训练权重再到指标的实际执行链路。

| 等级 | 链路完整度 | 复现含义 | 以 `Paper X / b1=82.7` 为例 |
|---|---|---|---|
| <strong>AAA</strong> | 完整闭环 | 已完整执行目标链路，完成全量审计以及必要的重复/不确定性分析，并保留可核查证据。 | 固定 checkpoint、完整测试划分、seed 和聚合方式后全量重跑得到 `82.6`，落在预先规定的容差内，逐样本结果和重复运行证据均已保存。 |
| <strong>AA</strong> | 基本闭环 | 已执行并审计完整主链路，只有不改变科学含义的少量可追溯兼容处理或确定性重建。 | 记录并应用只修改 API 调用、不改变模型与协议[^protocol]的兼容补丁后，全量重跑得到 `82.6`。 |
| <strong>A</strong> | 可执行但有重要限制 | 已完成主张级运行，但 seed、精确版本、checkpoint 选择或统计细节等重要条件无法与论文严格对齐。 | 作者没有公开 seed；复现者预先固定自行选用的 seed[^independent-seed] 并完成全量测试，得到 `82.5`，只能报告独立固定协议结果。 |
| <strong>BBB</strong> | 部分链路 | 已执行 evaluator replay、固定子集或独立协议验证，但尚未闭合论文原指标链路。 | 不加载模型，只把作者发布的预测文件送进评分脚本，重新算出 `82.7`（evaluator replay[^evaluator-replay]）。 |
| <strong>BB</strong> | 冒烟级 | 只完成代码导入、权重加载、单样本生成或 evaluator 接口运行。 | checkpoint 已成功加载，并在 `Dataset 1` 的一个样本上输出预测；尚未运行完整测试集。 |
| <strong>B</strong> | 仅工件追踪 | 已定位部分工件或入口，但尚未成功执行可产生指标的链路。 | 已下载仓库和权重，但尚未确认权重身份，也没有跑通测试命令。 |
| <strong>CCC</strong> | 严重缺失 | 因关键工件或协议缺失，实际复现无法开始或无法推进到可测试主张的程度。 | 缺少对应权重和测试划分，因而无法运行 `b1` 的测试链路。 |
| <strong>D</strong> | 无效链路 | 实际运行使用了错误身份或被替换的协议，结果已作废，证据不能归属于目标主张。 | 运行后发现使用的是另一模型变体或不同数据划分；所得数字不能归属于 `b1`。 |
| <strong>NR</strong> | 未评级/不适用 | 尚未执行或审计该复现维度，或该维度对当前主张不适用。 | 已登记 `b1=82.7`，但尚未开始运行测试链路。 |
