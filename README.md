...
| <strong>A</strong> | 可执行但有重要限制 | 可以进行主张级验证，但缺少 seed、精确版本、checkpoint 选择或统计细节等重要信息，不能称严格复现。 | checkpoint、测试代码和数据可用，但作者没有公开 seed；采用复现者自行选用的 seed[^independent-seed] 得到 `82.5`，只能称固定独立协议结果。 |
