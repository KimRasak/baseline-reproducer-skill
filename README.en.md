...
| <strong>AA</strong> | Substantially closed | The main chain is complete with only traceable compatibility handling or deterministic reconstruction that does not alter scientific meaning. | All official artifacts exist, but a dependency upgrade removed an old argument name. The compatibility patch only replaces it with the new argument name while keeping the value and computation unchanged; it changes neither the model nor the protocol[^protocol]. The rerun gives `82.6`. |
