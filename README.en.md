...
[^independent-seed]: A <strong>preregistered independent seed</strong> means that when the authors do not release their original random seeds, the reproducer chooses and records a seed set before running—for example, `42, 43, 44, 45, 46`—and then follows that plan. This prevents selecting only favorable seeds after seeing the results. Because these are not the authors' original seeds, the run is not an exact recovery of the original experiment.
